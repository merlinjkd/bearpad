	...
	function computeTheme(themeName: string) {
		...
		if (themeName === 'light') {
			return EditorView.theme({
				'&': { backgroundColor: '#ece7da', color: '#000000', height: '100%' },
		...
		return EditorView.theme({
			...
			'.cm-cursor': { borderLeft: '2px solid #fafafa' },
			            ...
			            '&.cm-focused > .cm-scroller > .cm-selectionLayer .cm-selectionBackground, &.cm-focused .cm-selectionLayer .cm-selectionBackground': { backgroundColor: '#264f78 !important' },
			            '.cm-content ::selection, .cm-content::selection': { color: '#ffffff' },
